   ...
   // public playerMoving playerMovingscript;
    ...
    private IEnumerator WaitForSleep()
    {
        ...
        if (Vector3.Dot(transform.forward, Vector3.up) > 0.8f)
            playerMovingscript.StartMovingWithAnimation(4);

        else if (Vector3.Dot(-transform.forward, Vector3.up) > 0.8f)        
            playerMovingscript.StartMovingWithAnimation(3);

        
        else if (Vector3.Dot(transform.right, Vector3.up) > 0.8f)        
            playerMovingscript.StartMovingWithAnimation(5);

        
        else if (Vector3.Dot(-transform.right, Vector3.up) > 0.8f)        
            playerMovingscript.StartMovingWithAnimation(2);

        
        else if (Vector3.Dot(transform.up, Vector3.up) > 0.8f)        
            playerMovingscript.StartMovingWithAnimation(1);

        
        else if (Vector3.Dot(-transform.up, Vector3.up) > 0.8f)        
            playerMovingscript.StartMovingWithAnimation(6);
